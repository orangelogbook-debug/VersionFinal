...
document.getElementById("searchHistory").onclick = async () => {
    ...
    db.collection("history")
        ...
        .onSnapshot((snapshot) => {
            ...
            /* ---------------------------------------------------------
               CHECK FOR WARNING (2 interventions in last 30 days)
            --------------------------------------------------------- */
            let recentCount = 0;
            const now = Date.now();
            ...
            snapshot.forEach(doc => {
                const h = doc.data();
                if (h.timestamp && (now - h.timestamp) <= THIRTY_DAYS) {
                    recentCount++;
                }
            });
            ...
            if (recentCount >= 2) {
                warningHtml = `
                    <div class="attentionWarning">
                        ⚠️ Machine needs attention — ${recentCount} interventions in the last 30 days
                    </div>
                `;
            }
