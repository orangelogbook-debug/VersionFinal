...
document.getElementById("saveIntervention").onclick = async () => {
    ...
    try {
        if (editingId) {
            // 🔥 On garde le timestamp original si on l’a
            ...
            // 🔥 Nouvelle intervention → timestamp basé sur la date saisie
            entry.timestamp = parsedDate.getTime();

            await db.collection("history").add(entry);
        }

        // Mise à jour de la localisation de la machine
...
document.getElementById("searchHistory").onclick = async () => {
    ...
    db.collection("history")
        ...
        .onSnapshot((snapshot) => {
            ...
            // 🔥 Empêche les snapshots instables (locaux) de réécrire le HTML
            ...
            /* ---------------------------------------------------------
               CHECK FOR WARNING (2 interventions within 30 days)
            --------------------------------------------------------- */

            // On récupère les timestamps triés DESC (déjà triés par Firestore)
            const timestamps = [];
            snapshot.forEach(doc => {
                const h = doc.data();
            ...
            if (timestamps.length >= 2) {
                const newest = timestamps[0];        // la plus récente
                const secondNewest = timestamps[1];  // la 2e plus récente
                ...
                if (diff <= THIRTY_DAYS) {
                    warningHtml = `
                        <div class="attentionWarning">
                            ⚠️ Machine needs attention — Many interventions in the last 30 days
                        </div>
                    `;
                }
            }
            ...
            /* ---------------------------------------------------------
               INJECT WARNING + RESULTS (UNE SEULE FOIS)
            --------------------------------------------------------- */
            const finalHtml = warningHtml + items.join("");
            ...
            document.querySelectorAll(".undoIcon").forEach(icon => {
                icon.addEventListener("click", async (e) => {
                    ...
                    try {
                        ...
                        // 🔥 On garde le timestamp original pour ne PAS changer la date
...
/* ---------------------------------------------------------
   FILTER INPUT (SEARCH INSIDE RESULTS)
--------------------------------------------------------- */
...
document.getElementById("exportPdf").onclick = async () => {
    ...
    pdfWindow.document.write(`
        ...
        <body>
            ...
            <div class="footer">
                https://orangelogbook-debug.github.io/VersionFinal/<br>
                ${today} — Page 1 sur 1
            </div>
